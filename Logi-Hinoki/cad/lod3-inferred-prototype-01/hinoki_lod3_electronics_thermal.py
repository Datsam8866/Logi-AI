...
def build_electronics_thermal(doc, groups):
    ...
    heat_shapes = {
        ...
        "Heat_IO": Part.makeBox(35.0, 20.0, 3.0, vector(353.5, 111.0, 28.6)),
    ...
    thermal_shapes = {
        ...
        "Copper_Spreader": Part.makeBox(100.0, 80.0, 2.0, vector(321.0, 206.0, 33.2)),
        "Heat_Pipe_Left": Part.makeCylinder(
            3.0, 160.0, vector(291.0, 224.0, 38.2), vector(1.0, 0.0, 0.0)
        ),
        "Heat_Pipe_Right": Part.makeCylinder(
            3.0, 160.0, vector(291.0, 268.0, 38.2), vector(1.0, 0.0, 0.0)
        ),
        "Aluminum_Interface": Part.makeBox(220.0, 120.0, 2.5, vector(261.0, 186.0, 41.2)),
    ...
    for index, (name, dimensions, position) in enumerate(connector_specs, start=70):
        parts.append(
            semantic_part(
                doc,
                group,
