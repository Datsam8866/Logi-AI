...
EXPORT_SCRIPT = CAD_ROOT / "export_iteration02_step.py"
CAD_FILE = CAD_ROOT / "Hinoki_Concept_CAD_Iteration02_Review.FCStd"
STEP_FILE = CAD_ROOT / "Hinoki_Concept_CAD_Iteration02_Concept.stp"
...
class HinokiIteration02Tests(unittest.TestCase):
    def test_iteration02_review_cad_contains_verified_baselines_and_open_risks(self):
        ...
        self.assertTrue(EXPORT_SCRIPT.exists(), "Iteration 02 STEP exporter must exist")
        ...
        run_freecad_script(EXPORT_SCRIPT)
        self.assertTrue(STEP_FILE.exists(), "Iteration 02 concept STEP must be created")
        self.assertGreater(STEP_FILE.stat().st_size, 1000)

        step_probe = """
import FreeCAD as App
import Import
import Part
doc = App.newDocument('Iteration02_STEP_Verification')
Import.insert(r'{step_file}', doc.Name)
doc.recompute()
shapes = [obj.Shape for obj in doc.Objects if hasattr(obj, 'Shape') and not obj.Shape.isNull()]
assert shapes, 'Imported STEP contains no shapes'
compound = Part.makeCompound(shapes)
assert len(compound.Solids) >= 6, 'Imported STEP contains fewer than six solids'
box = compound.BoundBox
assert box.XLength >= 750.0
assert box.YLength >= 300.0
assert box.ZLength >= 650.0
print('STEP import verification passed: solids={{}}, bbox={{:.1f}} x {{:.1f}} x {{:.1f}} mm'.format(
    len(compound.Solids), box.XLength, box.YLength, box.ZLength
))
""".format(step_file=STEP_FILE.as_posix())
        ...
        self.assertIn("STEP import verification passed", step_result.stdout)
...
""".format(cad_file=CAD_FILE.as_posix())
        ...
        self.assertEqual(review["step_export_status"], "ConceptReviewOnly")
        self.assertEqual(
